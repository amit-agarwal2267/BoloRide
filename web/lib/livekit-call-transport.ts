...
export class LiveKitCallTransport implements CallTransport {
  ...
  async connect(signal: AbortSignal): Promise<void> {
    ...
    try {
      ...
      const { serverUrl, participantToken } = await response.json(); check();
  ...
  async disconnect() { const room = this.room; this.room = undefined; this.clearAudio(); await room?.disconnect(); }
