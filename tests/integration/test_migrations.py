...
async def table_columns(database_url: str, table_name: str) -> set[str]:
    engine = create_async_engine(database_url)
    try:
        async with engine.connect() as connection:
            return set(
...
def test_clean_database_upgrade_downgrade_and_reupgrade(
    ...
    try:
        ...
        assert revision == "019"
        assert "model_name" in asyncio.run(table_columns(test_url, "vehicles"))
        assert "eta_minutes" in asyncio.run(table_columns(test_url, "ride_assignments"))
        command.downgrade(alembic_config, "016")
        assert "model_name" not in asyncio.run(table_columns(test_url, "vehicles"))
        assert "eta_minutes" not in asyncio.run(table_columns(test_url, "ride_assignments"))
        command.upgrade(alembic_config, "017")
        assert "model_name" in asyncio.run(table_columns(test_url, "vehicles"))
        assert "eta_minutes" in asyncio.run(table_columns(test_url, "ride_assignments"))
        assert {"users", "saved_places", "rides", "vehicle_types", "pricing_rules", "accepted_quotes", "offers", "offer_redemptions", "booking_attempts", "drivers", "vehicles", "ride_assignments", "demo_access_requests", "demo_phone_leases"}.issubset(tables)
        assert "pickup_instructions" in asyncio.run(table_columns(test_url, "rides"))

        command.downgrade(alembic_config, "015")
        assert asyncio.run(schema_state(test_url))[0] == "015"
        assert "pickup_instructions" not in asyncio.run(table_columns(test_url, "rides"))
        command.upgrade(alembic_config, "016")
        assert asyncio.run(schema_state(test_url))[0] == "016"
        assert "pickup_instructions" in asyncio.run(table_columns(test_url, "rides"))
        ...
        command.upgrade(alembic_config, "head")
        assert asyncio.run(schema_state(test_url))[0] == "019"
